...
- Local imports do not retain a live external source. Git and marketplace acquisitions may retain only portable Upstream Source metadata; a later explicit association may attach that metadata to a local import (ADR-0011). Updating a locally modified Skill requires explicit confirmation and a backup.
