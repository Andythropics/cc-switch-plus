...
├── skills/           # 非公開 Skill Library（macOS）
├── skill-import-backups/ # 更新・削除・移行の復旧データ
...
| prompts | プロンプトプリセット |
| library_skills | ポータブルな Library の識別情報、ソースメタデータ、スナップショット（macOS） |
| skill_repos | ポータブルな Skill リポジトリソース |
| project_workspaces | デバイスローカルな Project Workspace の識別情報（macOS） |
| skill_deployments | デバイスローカルな Claude/Codex Deployment の希望状態（macOS） |
| skill_activity | デバイスローカルな構造化変更履歴（macOS） |
| proxy_config | プロキシ設定 |
