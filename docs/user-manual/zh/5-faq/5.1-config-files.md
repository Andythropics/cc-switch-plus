...
├── settings.json     # 设备级设置
├── skills/           # 私有 Skill Library（macOS）
├── skill-import-backups/ # 更新、删除与迁移恢复数据
└── backups/          # 数据库备份
```
...
| prompts | 提示词预设 |
| library_skills | 可移植的 Library 身份、来源元数据与快照（macOS） |
| skill_repos | 可移植的 Skill 仓库来源 |
| project_workspaces | 设备本地的 Project Workspace 身份（macOS） |
| skill_deployments | 设备本地的 Claude/Codex Deployment 期望状态（macOS） |
| skill_activity | 设备本地的结构化变更历史（macOS） |
| proxy_config | 代理配置 |
