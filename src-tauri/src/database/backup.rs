...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        self.import_sql_string_inner_with_hook(sql_raw, preserve_tables, preserve_rows, || Ok(()))
    }
        ...
        F: FnOnce() -> Result<(), AppError>,
    {
        ...
        let schema_lineage = Self::detect_schema_lineage(&temp_conn)?;
        Self::normalize_schema_lineage_on_conn(&temp_conn, schema_lineage)?;
        // 补齐缺失表/索引并执行迁移
        ...
        let backup_path = {
            ...
            Self::restore_rows(&main_conn, &temp_conn, preserve_rows)?;
            #[cfg(target_os = "macos")]
    ...
    fn complete_backup(backup: &Backup<'_, '_>, context: &str) -> Result<(), AppError> {
        ...
        Self::ensure_backup_complete(result, context)
    }

    fn ensure_backup_complete(result: StepResult, context: &str) -> Result<(), AppError> {
        match result {
            StepResult::Done => Ok(()),
    ...
    pub(crate) fn backup_database_snapshot_file(&self, destination: &Path) -> Result<(), AppError> {
        ...
        let temp_path = temp.path().to_path_buf();
        {
            ...
            let backup = Backup::new(&conn, &mut destination_conn)
                .map_err(|error| AppError::Database(error.to_string()))?;
            Self::complete_backup(&backup, "创建数据库迁移快照")?;
        }
    ...
    pub(crate) fn restore_database_snapshot_file(&self, source: &Path) -> Result<(), AppError> {
        ...
        // Capture the lineage from the untouched snapshot. Eager current-schema
        // DDL below can create `session_usage_dedup`, which would otherwise make
        // an old macOS v17-v25 snapshot look canonical and skip its version
        // rebase.
        let source_lineage = Self::detect_schema_lineage(&source_conn)?;

        {
            let mut main_conn = lock_conn!(self.conn);
            let backup = Backup::new(&source_conn, &mut main_conn)
                .map_err(|error| AppError::Database(error.to_string()))?;
            Self::complete_backup(&backup, "恢复数据库迁移快照")?;
        }

        {
            let main_conn = lock_conn!(self.conn);
            Self::create_tables_on_conn(&main_conn)?;
            Self::apply_schema_migrations_on_conn_with_lineage(&main_conn, source_lineage)?;
        }
        ...
        F: FnOnce(Option<&Path>) -> Result<(), AppError>,
    {
        ...
        // Detect before eager DDL creates current tables and destroys the
        // distinguishing fingerprint of a pre-canonical macOS schema.
        let schema_lineage = Self::detect_schema_lineage(&staging_conn)?;
        Self::ensure_incremental_auto_vacuum_on_conn(&staging_conn)?;
        Self::create_tables_on_conn(&staging_conn)?;
        Self::apply_schema_migrations_on_conn_with_lineage(&staging_conn, schema_lineage)?;
...
mod tests {
    ...
    #[test]
    fn backup_completion_requires_done() {
        for (result, result_name) in [
            (StepResult::More, "More"),
            (StepResult::Busy, "Busy"),
            (StepResult::Locked, "Locked"),
        ] {
            let error = Database::ensure_backup_complete(result, "test snapshot")
                .expect_err("a non-Done backup result must fail");
            let message = error.to_string();
            assert!(
                message.contains("test snapshot未完成"),
                "error must include the operation context: {message}"
            );
            assert!(
                message.contains(result_name),
                "error must include the SQLite result: {message}"
            );
        }

        assert!(Database::ensure_backup_complete(StepResult::Done, "test snapshot").is_ok());
    }

    #[cfg(target_os = "macos")]
    fn write_legacy_macos_snapshot(path: &std::path::Path, version: i32) -> Result<(), AppError> {
        ...
        conn.execute_batch(
            "INSERT INTO providers (id, app_type, name, settings_config, meta)
             ...
             INSERT INTO library_skills (
                 ...
                 'legacy-library', 'review', 'Review', '{}', '{}', 'hash', 1, 2
             );",
        )?;
        if version >= 25 {
            conn.execute_batch(
                "INSERT INTO skills_migration_runs (
                     ...
                     'legacy-run', 'observation', 'resume', 'completed', 'plan', 3, 4
                 );
                 INSERT INTO skills_migration_findings (
                     ...
                     'legacy-run', 'finding', 'preserve', 'preserve_content',
                     'unmanaged', '[\"claude\"]', 'open', 3, 4
                 );",
            )?;
        }
    ...
    #[cfg(target_os = "macos")]
    fn assert_legacy_macos_snapshot_restored(db: &Database) -> Result<(), AppError> {
    ...
    #[test]
    #[serial]
    fn backup_restore_captures_legacy_lineage_before_eager_ddl() -> Result<(), AppError> {
        let test_home = TestHomeGuard::new();
        let backup_dir = test_home.path().join(".cc-switch/backups");
        std::fs::create_dir_all(&backup_dir).map_err(|error| AppError::io(&backup_dir, error))?;
        let filename = "legacy-v17.db";
        write_legacy_macos_snapshot(&backup_dir.join(filename), 17)?;

        let db = Database::memory()?;
        db.restore_from_backup(filename)?;

        assert_legacy_macos_snapshot_restored(&db)
    }

    #[cfg(target_os = "macos")]
    #[test]
    #[serial]
    fn protected_snapshot_restore_captures_legacy_lineage_before_eager_ddl() -> Result<(), AppError>
    {
        let test_home = TestHomeGuard::new();
        let snapshot = test_home.path().join("legacy-v25-snapshot.db");
        write_legacy_macos_snapshot(&snapshot, 25)?;
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM skills_migration_findings
                 WHERE run_id = 'legacy-run' AND finding_key = 'finding'",
                [],
                |row| row.get::<_, i64>(0),
            )?,
            1,
            "the final legacy report/journal rows must survive the version rebase"
        );
        Ok(())
    }
    ...
    fn every_sync_preserved_table_is_skipped_from_remote_payloads() {
        let preserved = super::platform_sync_tables(super::SYNC_PRESERVE_TABLES_BASE);
        let skipped = super::platform_sync_tables(super::SYNC_SKIP_TABLES_BASE);
        for table in preserved {
            assert!(
                skipped.contains(&table),
