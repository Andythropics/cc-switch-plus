...
// CC Switch Plus does not have its own update signing key or feed yet. These
// commands remain as compatibility guards for older frontend bundles, but must
// never query or install an upstream CC Switch release.
...
/// Compatibility guard for older frontends that still invoke app installation.
#[tauri::command]
pub async fn install_update_and_restart(_app: AppHandle) -> Result<bool, String> {
    Ok(false)
}

/// Compatibility guard for older frontends that still check for app updates.
#[tauri::command]
